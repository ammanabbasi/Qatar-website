...
export function FloatingWhatsApp({ audience, locale, label }: Props) {
  ...
  return (
    <a
      ...
      className={`plausible-event-name=whatsapp_floating_click plausible-event-audience=${audience} fixed bottom-5 end-5 z-30 inline-flex h-14 w-14 items-center justify-center gap-2.5 rounded-pill bg-(--color-brand) text-(--color-ink) shadow-[0_8px_24px_rgba(245,166,35,0.4)] transition-colors duration-200 ease-soft hover:bg-(--color-brand-hover) sm:w-auto sm:ps-4 sm:pe-5`}
    >
